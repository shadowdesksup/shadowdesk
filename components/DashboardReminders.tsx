        ...
        {lembretesFuturos.length === 0 ? (
          ...
          <div className="grid grid-cols-3 gap-5">
            {lembretesFuturos.map((lembrete, index) => {
              ...
              // Rotação aleatória mas consistente
              const rotacao = (lembrete.id.charCodeAt(0) % 7) - 3;
              ...
              return (
                <motion.div
                  ...
                  className="aspect-square relative group cursor-pointer will-change-transform"
                  style={{
                ...
                >
                  ...
                  >
                    <div className="relative transform scale-75 origin-top transition-all duration-300 group-hover/pin:scale-90 group-hover/pin:brightness-110 group-hover/pin:drop-shadow-[0_0_8px_rgba(0,0,0,0.2)] active:scale-95">
                      {/* Sombra do alfinete */}
                      <div className="absolute top-2 left-1/2 -translate-x-1/2 w-4 h-4 bg-black/20 rounded-full blur-sm" />

                      {/* Cabeça do alfinete */}
                      <div className={`w-6 h-6 ${estilos.pin} rounded-full shadow-lg relative transition-colors duration-300`}>
                      ...
                      {/* Tooltip */}
                      <div className="absolute -top-8 left-1/2 -translate-x-1/2 bg-red-500 text-white text-[10px] px-2 py-0.5 rounded shadow w-max whitespace-nowrap font-bold transition-all duration-200 pointer-events-none opacity-0 translate-y-2 group-hover/pin:opacity-100 group-hover/pin:translate-y-0">
                        Excluir
                      </div>
                    </div>
                  </div>
                  ...
                  <div
                    className={`w-full h-full p-3 flex flex-col justify-between relative shadow-lg transition-shadow duration-300 hover:shadow-xl overflow-hidden rounded-xl ${estilos.text} ${estilos.border} border`}
                    style={{
                      background: `linear-gradient(135deg, ${estilos.gradientStart} 0%, ${estilos.gradientEnd} 100%)`,
                      fontFamily: "'Caveat', cursive",
                      outline: '1px solid transparent' // Hack para evitar serrilhado em rotação
                    }}
                  >
                    ...
                    <div className="mt-2 w-full">
                      <h4
                        className="font-bold text-xl leading-tight mb-2 truncate text-center w-full"
                        title={lembrete.titulo}
                      >
                        {lembrete.titulo}
                      </h4>

                      {/* Descrição simulando anotação - Ajustada para cores dinamicas */}
                      {lembrete.descricao && (
                        <div
                          className={`text-base leading-5 font-bold overflow-hidden relative w-full break-words text-justify ${estilos.text} opacity-90`}
                          style={{
                            backgroundImage: 'repeating-linear-gradient(transparent, transparent 17px, rgba(0,0,0,0.1) 18px, rgba(0,0,0,0.1) 20px)', // Linhas mais grossas e escuras
                    ...
                    {/* Date Footer - Badge arredondado e fixo */}
                    <div
                      className="absolute bottom-3 left-1/2 -translate-x-1/2 w-max max-w-[90%] z-20"
                      style={{ backfaceVisibility: 'hidden', transform: 'translateX(-50%) translateZ(0)' }}
                    >
                      <div className="flex items-center justify-center gap-2 font-sans text-[10px] bg-white/50 p-1.5 px-3 rounded-full shadow-sm text-center border border-white/20 backdrop-blur-sm antialiased">
                        ...
                        <span className="w-px h-2.5 bg-black/20"></span>
                        <div className="flex items-center gap-1">
                    ...
                    <div
                      ...
                      style={{
                        background: 'linear-gradient(135deg, transparent 50%, rgba(0,0,0,0.1) 50%, rgba(0,0,0,0.05) 53%, transparent 55%)',
                        pointerEvents: 'none'
                      }}
                    />
                  </div>
