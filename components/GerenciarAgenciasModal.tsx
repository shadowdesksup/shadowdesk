...
import { Plus, Trash2, Edit2, Briefcase, Save, X } from 'lucide-react';
import { AgenciaProjeto } from '../types';
import { listarAgenciasProjeto, criarAgenciaProjeto, deletarAgenciaProjeto, atualizarAgenciaProjeto } from '../firebase/agenciasProjeto';

interface GerenciarAgenciasModalProps {
  isOpen: boolean;
  onClose: () => void;
  theme?: 'dark' | 'light';
}

const GerenciarAgenciasModal: React.FC<GerenciarAgenciasModalProps> = ({ isOpen, onClose, theme = 'dark' }) => {
  const isDark = theme === 'dark';
  
  const [agencias, setAgencias] = useState<AgenciaProjeto[]>([]);
  ...
  const carregar = async () => {
    try {
      setCarregando(true);
      const data = await listarAgenciasProjeto();
      setAgencias(data);
    } catch (err: any) {
      setErro(err.message);
    } finally {
  ...
  const handleAdd = async (e: React.FormEvent) => {
    ...
    try {
      setCarregando(true);
      await criarAgenciaProjeto(novoNome);
      setNovoNome('');
      await carregar();
    } catch (err: any) {
      setErro(err.message);
  ...
  const handleEditSave = async (id: string) => {
    ...
    try {
      setCarregando(true);
      await atualizarAgenciaProjeto(id, editNome);
      setEditandoId(null);
      await carregar();
    } catch (err: any) {
      setErro(err.message);
  ...
  const handleDelete = async (id: string) => {
    if (!window.confirm('Excluir esta agência? Ela deixará de aparecer nas opções do banco.')) return;
    setErro(null);
    try {
      setCarregando(true);
      await deletarAgenciaProjeto(id);
      await carregar();
    } catch (err: any) {
      setErro(err.message);
      setCarregando(false);
          ...
          >
            <div className={`p-5 flex items-center justify-between border-b ${isDark ? 'border-slate-800 bg-slate-900/80' : 'border-slate-200 bg-slate-50'}`}>
              <h2 className={`text-xl font-bold flex items-center gap-2 ${isDark ? 'text-white' : 'text-slate-800'}`}>
                <Briefcase className="text-cyan-500" /> Banco de Agências
              </h2>
              <button 
                onClick={onClose}
            ...
            <div className="p-6">
              ...
              <p className={`text-sm mb-6 ${isDark ? 'text-slate-400' : 'text-slate-600'}`}>
                Gerencie as Agências de Fomento (ex: CNPq, FAPESP, CAPES) para os projetos de pesquisa.
              </p>
              ...
              <form onSubmit={handleAdd} className="flex gap-2 mb-8">
                <input
                  ...
                  placeholder="Nova agência (ex: CNPq)"
              ...
              <h3 className={`font-semibold mb-3 ${isDark ? 'text-slate-300' : 'text-slate-700'}`}>Agências na Base</h3>
