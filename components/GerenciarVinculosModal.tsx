...
import { Plus, Trash2, Edit2, Link2, Save, X } from 'lucide-react';
import { VinculoEquipamento, listarVinculosEquipamento, criarVinculoEquipamento, deletarVinculoEquipamento, atualizarVinculoEquipamento } from '../firebase/vinculosEquipamento';

interface GerenciarVinculosModalProps {
  isOpen: boolean;
  onClose: () => void;
  theme?: 'dark' | 'light';
}

const GerenciarVinculosModal: React.FC<GerenciarVinculosModalProps> = ({ isOpen, onClose, theme = 'dark' }) => {
  ...
  const carregar = async () => {
    try {
      setCarregando(true);
      const data = await listarVinculosEquipamento();
      setVinculos(data);
    } catch (err: any) {
      setErro(err.message);
    } finally {
  ...
  const handleAdd = async (e: React.FormEvent) => {
    ...
    try {
      setCarregando(true);
      await criarVinculoEquipamento(novoNome);
      setNovoNome('');
      await carregar();
    } catch (err: any) {
      setErro(err.message);
  ...
  const handleEditSave = async (id: string) => {
    ...
    try {
      setCarregando(true);
      await atualizarVinculoEquipamento(id, editNome);
      setEditandoId(null);
      await carregar();
    } catch (err: any) {
      setErro(err.message);
  ...
  const handleDelete = async (id: string) => {
    if (!window.confirm('Excluir este vínculo? Ele deixará de aparecer nas opções do banco.')) return;
    setErro(null);
    try {
      setCarregando(true);
      await deletarVinculoEquipamento(id);
      await carregar();
    } catch (err: any) {
      setErro(err.message);
      setCarregando(false);
          ...
          >
            <div className={`p-5 flex items-center justify-between border-b ${isDark ? 'border-slate-800 bg-slate-900/80' : 'border-slate-200 bg-slate-50'}`}>
              <h2 className={`text-xl font-bold flex items-center gap-2 ${isDark ? 'text-white' : 'text-slate-800'}`}>
                <Link2 className="text-cyan-500" /> Banco de Vínculos
              </h2>
              <button 
                onClick={onClose}
            ...
            <div className="p-6">
              ...
              <p className={`text-sm mb-6 ${isDark ? 'text-slate-400' : 'text-slate-600'}`}>
                Adicione ou remova Vínculos Padrões (ex: CLT, Terceirizado, Estagiário, Bolsista).
              </p>
              ...
              <form onSubmit={handleAdd} className="flex gap-2 mb-8">
                <input
                  ...
                  placeholder="Novo vínculo (ex: CLT)"
              ...
              <h3 className={`font-semibold mb-3 ${isDark ? 'text-slate-300' : 'text-slate-700'}`}>Vínculos Fixos na Base</h3>
              <div className={`overflow-y-auto rounded-xl border custom-scrollbar ${isDark ? 'border-slate-800 bg-slate-900/50' : 'border-slate-200 bg-slate-50'} max-h-[40vh]`}>
                {carregando && !vinculos.length ? (
                  ...
                  <div className={`p-6 text-center text-sm ${isDark ? 'text-slate-500' : 'text-slate-400'}`}>
                    Nenhum vínculo padrão cadastrado.
                  </div>
                ) : (
