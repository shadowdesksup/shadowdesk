...
import { Plus, Trash2, Edit2, Server, Save, X } from 'lucide-react';
import { TipoEquipamento } from '../types';
import { listarTiposEquipamento, criarTipoEquipamento, deletarTipoEquipamento, atualizarTipoEquipamento } from '../firebase/tiposEquipamento';
...
const GerenciarTiposModal: React.FC<GerenciarTiposModalProps> = ({ isOpen, onClose, theme = 'dark' }) => {
  ...
  const carregar = async () => {
    try {
      setCarregando(true);
      const data = await listarTiposEquipamento();
      setTipos(data);
    } catch (err: any) {
      setErro(err.message);
    } finally {
  ...
  const handleAdd = async (e: React.FormEvent) => {
    ...
    try {
      setCarregando(true);
      await criarTipoEquipamento(novoNome);
      setNovoNome('');
      await carregar();
    } catch (err: any) {
      setErro(err.message);
  ...
  const handleEditSave = async (id: string) => {
    ...
    try {
      setCarregando(true);
      await atualizarTipoEquipamento(id, editNome);
      setEditandoId(null);
      await carregar();
    } catch (err: any) {
      setErro(err.message);
  ...
  const handleDelete = async (item: TipoEquipamento) => {
    if (!window.confirm(`Excluir o tipo "${item.nome}"? Ele deixará de aparecer nas opções do banco.`)) return;
    setErro(null);
    try {
      setCarregando(true);
      const emUso = await verificarUsoEmEstoque('tipo', item.nome);
      if (emUso) {
        setErro(`Não é possível excluir: existem equipamentos cadastrados com o tipo "${item.nome}".`);
        setCarregando(false);
        return;
      }
      await deletarTipoEquipamento(item.id);
      await carregar();
    } catch (err: any) {
      setErro(err.message);
      setCarregando(false);
          ...
          >
            ...
            <div className={`p-5 flex items-center justify-between border-b ${isDark ? 'border-slate-800 bg-slate-900/80' : 'border-slate-200 bg-slate-50'}`}>
              <h2 className={`text-xl font-bold flex items-center gap-2 ${isDark ? 'text-white' : 'text-slate-800'}`}>
                <Server className="text-cyan-500" /> Banco de Tipos
              </h2>
              <button 
                onClick={onClose}
            ...
            <div className="p-6">
              ...
              <p className={`text-sm mb-6 ${isDark ? 'text-slate-400' : 'text-slate-600'}`}>
                Adicione ou remova Tipos Padrões para ajudar o preenchimento dos usuários (autocompletar).
              </p>
              ...
              <form onSubmit={handleAdd} className="flex gap-2 mb-8">
                <input
                  ...
                  placeholder="Novo tipo (ex: Switch)"
              ...
              <h3 className={`font-semibold mb-3 ${isDark ? 'text-slate-300' : 'text-slate-700'}`}>Tipos Fixos na Base</h3>
