...
import { Plus, Trash2, Edit2, Cpu, Save, X } from 'lucide-react';
import { ModeloEquipamento } from '../types';
import { listarModelosEquipamento, criarModeloEquipamento, deletarModeloEquipamento, atualizarModeloEquipamento } from '../firebase/modelosEquipamento';
import { verificarUsoEmEstoque } from '../firebase/estoque';

interface GerenciarModelosModalProps {
  isOpen: boolean;
  onClose: () => void;
  theme?: 'dark' | 'light';
}

const GerenciarModelosModal: React.FC<GerenciarModelosModalProps> = ({ isOpen, onClose, theme = 'dark' }) => {
  ...
  const carregar = async () => {
    try {
      setCarregando(true);
      const data = await listarModelosEquipamento();
      setModelos(data);
    } catch (err: any) {
      setErro(err.message);
    } finally {
  ...
  const handleAdd = async (e: React.FormEvent) => {
    ...
    try {
      setCarregando(true);
      await criarModeloEquipamento(novoNome);
      setNovoNome('');
      await carregar();
    } catch (err: any) {
      setErro(err.message);
  ...
  const handleEditSave = async (id: string) => {
    ...
    try {
      setCarregando(true);
      await atualizarModeloEquipamento(id, editNome);
      setEditandoId(null);
      await carregar();
    } catch (err: any) {
      setErro(err.message);
  ...
  const handleDelete = async (item: ModeloEquipamento) => {
    if (!window.confirm(`Excluir o modelo "${item.nome}"? Ele deixará de aparecer nas opções do banco.`)) return;
    setErro(null);
    try {
      setCarregando(true);
      const emUso = await verificarUsoEmEstoque('modelo', item.nome);
      if (emUso) {
        setErro(`Não é possível excluir: existem equipamentos cadastrados com o modelo "${item.nome}".`);
        setCarregando(false);
        return;
      }
      await deletarModeloEquipamento(item.id);
      await carregar();
    } catch (err: any) {
      setErro(err.message);
      setCarregando(false);
          ...
          >
            <div className={`p-5 flex items-center justify-between border-b ${isDark ? 'border-slate-800 bg-slate-900/80' : 'border-slate-200 bg-slate-50'}`}>
              <h2 className={`text-xl font-bold flex items-center gap-2 ${isDark ? 'text-white' : 'text-slate-800'}`}>
                <Cpu className="text-cyan-500" /> Banco de Modelos
              </h2>
              <button 
                onClick={onClose}
            ...
            <div className="p-6">
              ...
              <p className={`text-sm mb-6 ${isDark ? 'text-slate-400' : 'text-slate-600'}`}>
                Adicione ou remova Modelos Padrões para ajudar o preenchimento dos usuários (autocompletar).
              </p>
              ...
              <form onSubmit={handleAdd} className="flex gap-2 mb-8">
                <input
                  ...
                  placeholder="Novo modelo (ex: Optiplex)"
              ...
              <h3 className={`font-semibold mb-3 ${isDark ? 'text-slate-300' : 'text-slate-700'}`}>Modelos Fixos na Base</h3>
              <div className={`overflow-y-auto rounded-xl border custom-scrollbar ${isDark ? 'border-slate-800 bg-slate-900/50' : 'border-slate-200 bg-slate-50'} max-h-[40vh]`}>
                {carregando && !modelos.length ? (
                  ...
                  <div className={`p-6 text-center text-sm ${isDark ? 'text-slate-500' : 'text-slate-400'}`}>
                    Nenhum modelo padrão cadastrado.
                  </div>
                ) : (
