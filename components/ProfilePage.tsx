...
import { User as UserIcon, Lock, Save, AlertCircle, CheckCircle2, Trash2 } from 'lucide-react';
...
const ProfilePage: React.FC<ProfilePageProps> = ({ usuario, dadosUsuario, theme = 'dark' }) => {
  ...
  // AUTO-SAVE: Salva telefone automaticamente quando toggle ON + número válido
  React.useEffect(() => {
    // Só salva se toggle ON e número tem pelo menos 10 dígitos
    if (!whatsappEnabled || whatsappPhone.length < 10) return;
    ...
    const timer = setTimeout(async () => {
      try {
        ...
        console.log('✅ Telefone salvo automaticamente!');
      } catch (e) {
        console.warn('Falha ao auto-salvar telefone:', e);
      }
        ...
        <div className="flex flex-col gap-6">
          ...
          <div className="flex items-center justify-between">
            ...
            <div
              ...
              onClick={async () => {
                ...
                if (newState && whatsappPhone.trim().length < 10) {
                  setErroWhatsapp('Informe o número de WhatsApp primeiro (mínimo 10 dígitos)');
                  return;
                }
                ...
                try {
                  ...
                  if (newState) {
                    // ATIVANDO: reseta individuais para herdar do Main + salva telefone
                    await updateDoc(doc(db, 'users', usuario.uid), {
                      whatsappLembretesEnabled: true,
                      whatsappServiceDeskEnabled: true,
                      whatsappLembreteModalEnabled: true,
                      telefone: whatsappPhone.trim()
                    });
                  } else {
                    // DESATIVANDO: desliga tudo
                    await updateDoc(doc(db, 'users', usuario.uid), {
                      whatsappLembretesEnabled: false,
                      whatsappServiceDeskEnabled: false,
                      whatsappLembreteModalEnabled: false
                    });
                  }
                  setSucessoWhatsapp(newState ? 'Notificações ativadas!' : 'Notificações desativadas');
                  setTimeout(() => setSucessoWhatsapp(''), 3000);
                } catch (e) {
                  console.warn('Falha ao salvar toggle:', e);
                  setErroWhatsapp('Erro ao salvar preferência');
                }
              }}
            >
              <motion.div
          ...
          <div className="flex flex-col gap-2">
            ...
            <p className={`text-xs ${theme === 'dark' ? 'text-slate-500' : 'text-slate-400'}`}>
              Digite apenas números (DDD + número). Ex: (14)99999-9999
            </p>
          </div>
          ...
          {/* Botão Remover Número - compacto e alinhado à direita */}
          {whatsappPhone && (
            <div className="flex justify-end">
              <motion.button
                ...
                className={`px-4 py-2.5 rounded-xl font-bold text-sm flex items-center gap-2 border transition-colors ${theme === 'dark'
      ...
      <ConfirmationModal
        ...
        onConfirm={async () => {
          try {
            ...
            setSucessoWhatsapp('Número removido com sucesso!');
            setTimeout(() => setSucessoWhatsapp(''), 3000);
          } catch (e) {
            setErroWhatsapp('Erro ao remover número');
          }
        }}
        title="Remover Número"
        message="Tem certeza que deseja remover seu número? Isso vai desativar todas as notificações de WhatsApp."
