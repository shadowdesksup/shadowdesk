...
const EstoqueMovimentacaoModal: React.FC<EstoqueMovimentacaoModalProps> = ({
  isOpen, onClose, estoqueAtivo, theme = 'dark', onAbreGerenciarOrigens, onAbreGerenciarVinculos, preSelectedItem
}) => {
  ...
  const locaisFilter = locaisBanco.filter(l => l.nome.toLowerCase().includes(localDestinoNome.toLowerCase()));
  const vinculosFilter = vinculosBanco.filter(v => v.nome.toLowerCase().includes(vinculoDestinoNome.toLowerCase()));
    ...
    try {
       ...
       await atualizarEquipamento(itemSelecionado.id, {
         ...
         historico: [
           ...
           {
             acao: `Transferido definitivamente para: ${localNome} (Resp: ${recebedorNome}${vinculoNome ? ' - '+vinculoNome : ''}). Motivo: ${motivo}`,
  ...
  return (
    <EstoqueSidePanel isOpen={isOpen} onClose={onClose} theme={theme} title={titleNode} width="md:w-[calc(100vw-8rem)] lg:w-[calc(100vw-16rem)] max-w-none">
      <div className="flex flex-col lg:flex-row min-h-full h-full bg-transparent">
        ...
        <div className={`w-full lg:w-1/2 p-6 overflow-y-auto ${isDark ? 'border-r border-slate-700/50' : 'border-r border-slate-200'} custom-scrollbar`} style={{ maxHeight: '100%' }}>
            ...
            {tiposBanco.map(tipo => {
               ...
               return (
                 <button
                   ...
                   className={`flex justify-between items-center px-4 py-3 rounded-xl border text-left transition-all ${
                 ...
                 >
                   ...
                   <span className={`text-[11px] font-bold px-2 py-0.5 rounded-full ${
                     tipoSelecionadoId === tipo.id ? 'bg-white/20 text-white' : isDark ? 'bg-slate-700 text-slate-300' : 'bg-slate-200 text-slate-600'
                   }`}>{contagem}</span>
                 </button>
               )
            })}
        ...
        <div className={`w-full lg:w-1/2 p-4 md:p-6 bg-transparent flex flex-col ${!itemSelecionado ? 'opacity-50 items-center justify-center' : ''}`}>
           {!itemSelecionado ? (
              ...
              <div className="w-full h-full flex flex-col">
                 <div className={`w-full flex-1 rounded-3xl p-6 md:p-8 border shadow-2xl flex flex-col overflow-y-auto custom-scrollbar ${isDark ? 'bg-slate-800/80 border-slate-700' : 'bg-white border-slate-200'}`}>
                    ...
                    <div className="space-y-5 flex-1 flex flex-col">
                            ...
                            <div className="flex gap-2 relative">
                               <div className="relative flex-1">
                                  <input 
                                    ...
                                    onChange={e => { setLocalDestinoNome(e.target.value); setShowLocaisDropdown(true); }}
                                    onFocus={() => setShowLocaisDropdown(true)}
                                  ...
                                  {showLocaisDropdown && locaisFilter.length > 0 && (
                                     <div className={`absolute z-50 w-full mt-1 rounded-xl border shadow-2xl max-h-48 overflow-y-auto ${isDark ? 'bg-slate-800 border-slate-600' : 'bg-white border-slate-200'} custom-scrollbar`}>
                                        {locaisFilter.map(l => (
                                          <div key={l.id} onMouseDown={() => setLocalDestinoNome(l.nome)} className={`px-4 py-3 cursor-pointer ${isDark ? 'hover:bg-slate-700 text-slate-200' : 'hover:bg-slate-50 text-slate-700'}`}>
                                             {l.nome}
                                          </div>
                                        ))}
                                     </div>
                               ...
                               <button onClick={onAbreGerenciarOrigens} title="Gerenciar Locais" className={`w-12 shrink-0 rounded-xl border flex items-center justify-center transition-all ${isDark ? 'bg-slate-800 border-slate-700 text-cyan-400 hover:bg-slate-700' : 'bg-slate-100 border-slate-300 text-cyan-600 hover:bg-slate-200'}`}>
                                  <Plus size={20} />
                               </button>
                            </div>
                          </div>
                            ...
                            <div className="flex gap-2 relative">
                               <div className="relative flex-1">
                                  <input 
                                    ...
                                    onChange={e => { setVinculoDestinoNome(e.target.value); setShowVinculosDropdown(true); }}
                                    onFocus={() => setShowVinculosDropdown(true)}
                                    onBlur={() => setTimeout(() => setShowVinculosDropdown(false), 200)}
                                    className={`w-full rounded-xl px-4 py-3 outline-none transition-all ${isDark ? 'bg-slate-900 border-slate-700 text-white focus:border-cyan-500' : 'bg-slate-50 border-slate-300 text-slate-800 focus:border-cyan-500'} border`}
                                  />
                                  {showVinculosDropdown && vinculosFilter.length > 0 && (
                                     <div className={`absolute z-50 w-full mt-1 rounded-xl border shadow-2xl max-h-48 overflow-y-auto ${isDark ? 'bg-slate-800 border-slate-600' : 'bg-white border-slate-200'} custom-scrollbar`}>
                                        {vinculosFilter.map(v => (
                                          <div key={v.id} onMouseDown={() => setVinculoDestinoNome(v.nome)} className={`px-4 py-3 cursor-pointer ${isDark ? 'hover:bg-slate-700 text-slate-200' : 'hover:bg-slate-50 text-slate-700'}`}>
                                             {v.nome}
                                          </div>
                                        ))}
                                     </div>
                               ...
                               <button onClick={onAbreGerenciarVinculos} title="Gerenciar Vínculos" className={`w-12 shrink-0 rounded-xl border flex items-center justify-center transition-all ${isDark ? 'bg-slate-800 border-slate-700 text-cyan-400 hover:bg-slate-700' : 'bg-slate-100 border-slate-300 text-cyan-600 hover:bg-slate-200'}`}>
                                  <Plus size={20} />
                               </button>
                            </div>
                          </div>
                       ...
                       <button 
                         ...
                         disabled={salvando}
                         className={`w-full mt-4 flex justify-center items-center gap-3 px-6 py-4 rounded-xl text-white font-bold text-lg transition-all ${
                           salvando ? 'bg-cyan-600 opacity-70 cursor-not-allowed' : 'bg-gradient-to-r from-cyan-400 to-blue-500 hover:from-cyan-300 hover:to-blue-400 shadow-[0_0_20px_rgba(6,182,212,0.4)] hover:shadow-[0_0_30px_rgba(6,182,212,0.6)] hover:-translate-y-1'
                         }`}
                       >
                          {salvando ? (
