...
const EstoqueMovimentadosModal: React.FC<EstoqueMovimentadosModalProps> = ({
  isOpen, onClose, estoque, theme = 'dark', onRealocar, highlightItemId, hidden = false
}) => {
  ...
  return (
    <>
      <EstoqueSidePanel isOpen={isOpen} onClose={onClose} theme={theme} title={titleNode} width="md:w-[calc(100vw-8rem)] lg:w-[calc(100vw-16rem)] max-w-none" hidden={hidden || itemSelecionado !== null}>
                ...
                filtrados.map((item, index) => {
                  ...
                  return (
                    <motion.div
                      ...
                      className={`relative flex flex-row items-start cursor-pointer gap-3 sm:gap-4 py-3 sm:py-4 px-3 sm:px-4 rounded-xl border-b last:border-b-0 transition-all overflow-hidden ${isHighlighted
                          ...
                          <div className="flex-1 min-w-0 pr-2">
                            ...
                            <div className="flex flex-wrap gap-1.5 text-[10px] sm:text-xs mt-0.5 font-mono items-center">
                              {item.patrimonio && item.patrimonio !== 'Sem informação'
                                ? <span className={`text-cyan-500 font-medium`}>PT: {item.patrimonio}</span>
                                : item.numeroSerie && item.numeroSerie !== 'Sem informação'
                                  ? <span className={`text-cyan-500 font-medium`}>NS: {item.numeroSerie}</span>
                                  : null
                              }
                            </div>
                          </div>
