...
const ServiceDeskNotificationModal: React.FC<ServiceDeskNotificationModalProps> = ({
  ...
  // Estado vem APENAS dos props (Firebase)
  const [enabled, setEnabled] = useState(initialEnabled);
  const [telefone, setTelefone] = useState(initialPhone);

  // Sincroniza quando props mudam (ex: após salvar no Firebase)
  useEffect(() => {
  ...
  const handleSalvar = async () => {
    ...
    try {
      ...
      // SYNC COM PERFIL GLOBAL: Atualizar o perfil do usuário no Firebase
      if (enabled && telefone.trim()) {
        try {
          ...
          if (auth.currentUser) {
            await updateDoc(doc(db, 'users', auth.currentUser.uid), {
              ...
              whatsappLembretesEnabled: true  // Ativa o toggle principal do perfil!
            });
          }
        } catch (e) {
    ...
    <AnimatePresence>
      ...
      >
        ...
        >
              ...
              <div className="flex items-center justify-between">
                ...
                <div
                  ...
                  onClick={async () => {
                    ...
                    // Se tentando ATIVAR sem telefone válido, mostra erro
                    if (newState && telefone.trim().length < 10) {
                      setErro('Informe o número de WhatsApp primeiro');
                      return;
                    }
                    setEnabled(newState);
                    setErro(null);
                      ...
                      if (auth.currentUser) {
                        if (newState) {
                          ...
                          await updateDoc(doc(db, 'users', auth.currentUser.uid), {
                            ...
                            whatsappServiceDeskEnabled: true,     // Este toggle
                            whatsappLembreteModalEnabled: true,   // Propaga para Lembrete também
                            telefone: telefone.trim()
                          });
                        } else {
                          // Desativando: só desliga ESTE toggle
                          await updateDoc(doc(db, 'users', auth.currentUser.uid), {
          ...
          <div className={`flex justify-center p-4 border-t shrink-0 ${theme === 'dark' ? 'border-white/10' : 'border-slate-200'}`}>
            <motion.button
              whileHover={{ scale: 1.02 }}
              whileTap={{ scale: 0.98 }}
              onClick={onClose}
              className={`w-full py-2.5 rounded-lg font-medium text-sm ${theme === 'dark'
